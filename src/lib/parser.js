...
export function parseDefaultObjectValue (schema, fields, value) {
  ...
  fields.forEach(field => {
    ...
    const itemValue =
      field.schemaType === SCHEMA_TYPES.BOOLEAN
    ...
    switch (field.schemaType) {
      ...
      default:
        if (field.isArrayField) {
          ...
          field.itemsNum =
            type === INPUT_TYPES.CHECKBOX ? field.items.length : field.minItems
        } else {
          data[name] = parseEventValue(eventInput)
        }
        break
...
export function loadFields (schema, fields, name = null, model = null) {
  switch (schema.type) {
    case SCHEMA_TYPES.OBJECT:
      ...
      const properties =
        schema.order instanceof Array ? schema.order : allProperties
...
export function parseArray (schema, name = null, model = null) {
  ...
  loop: for (let keyword of ARRAY_KEYWORDS) {
    if (schema.hasOwnProperty(keyword)) {
      switch (keyword) {
        case 'enum':
          if (!field.attrs.type) {
  ...
  if (!field.attrs.type) {
    field.isArrayField = true
    field.attrs.type =
      schema.items && NUMBER_TYPES.includes(schema.items.type)
        ? schema.items.type
        : INPUT_TYPES.TEXT
